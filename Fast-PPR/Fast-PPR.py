...
def get_in_neighbors(G, u):
    mask = G[1] == u
    return G[:, mask][0]

def get_out_degree(G, u):
...
class PPR:
    ...
    def fast_ppr(self, s, t):
        ...
        if s in t_set:
            ...
            return sum_pi_inv/number_of_walks
    ...
    def _frontier(self, t):
        ...
        while residual_vec_bigger_ix.size(dim=0) > 0:
            w = residual_vec_bigger_ix[0]  #maybe need to add item here
            for u in get_in_neighbors(self._G, w):
                capital_delta = (1-self._alpha)*residual_vec[w]/get_out_degree(self._G, u)
                estimate_vec[u] = estimate_vec[u] + capital_delta
                residual_vec[u] = residual_vec[u] + capital_delta
                if estimate_vec[u] > error_inv:
                    target_set.add(u)
                    frontier_set = set(get_in_neighbors(self._G, u).tolist()).union(frontier_set)
            residual_vec[w] = 0
            residual_vec_bigger_ix = (residual_vec > error_inv * self._alpha).nonzero().flatten()
